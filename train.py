...
from xlm.model import check_model_params, build_model, build_language_discriminator
...
def get_parser():
    ...
    parser.add_argument("--use_language_discriminator", type=bool_flag, default=False,
                        help="whether use the language discriminator for the BT training")
    parser.add_argument("--language_discriminator_loss_weight", type=float, default=1,
                        help="the weight for the language discriminator loss")
    parser.add_argument("--n_layer_dis", type=int, default=1)
    parser.add_argument("--dropout_dis", type=float, default=0.2)
    parser.add_argument("--optimizer_dis", type=str, default="adam,lr=0.00001",
                        help="Optimizer (SGD / RMSprop / Adam, etc.)")
...
def main(params):
    ...
    if params.encoder_only:
        ...
        if params.use_language_discriminator == False:
            trainer = EncDecTrainer(encoder, decoder, data, params)
            evaluator = EncDecEvaluator(trainer, data, params)
        else:
            language_discriminator = build_language_discriminator(params)
            trainer = EncDecTrainer(encoder, decoder, data, params, language_discriminator=language_discriminator)
            evaluator = EncDecEvaluator(trainer, data, params)
    # evaluation
    if params.eval_only:
    ...
    for _ in range(params.max_epoch):
        ...
        while trainer.n_sentences < trainer.epoch_size:
            ...
            # logger.info(sum(p.sum().item() for p in encoder.parameters()))
            # logger.info(sum(p.sum().item() for p in decoder.parameters()))
            # logger.info(sum(p.sum().item() for p in language_discriminator.parameters()))
