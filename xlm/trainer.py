...
class Trainer(object):
    ...
    def set_parameters(self):
        ...
        named_params_dis = []
        for name in self.MODEL_NAMES:
            if name != 'language_discriminator':
        ...
        if params.use_language_discriminator:
            named_params_dis.extend([(k, p) for k, p in getattr(self, 'language_discriminator').named_parameters() if p.requires_grad])
            self.parameters['language_discriminator'] =  [p for k, p in named_params_dis if not k.endswith(HashingMemory.MEM_VALUES_PARAMS)]
    ...
    def set_optimizers(self):
        ...
        # optimizer for language_discriminator
        if params.use_language_discriminator:
            # simple adam optimizer can be used for the language discriminator 
            self.optimizers['language_discriminator'] = get_optimizer(self.parameters['language_discriminator'], params.optimizer_dis)
    ...
    def optimize(self, loss, model_name='model'):
        ...
        assert model_name in ['model', 'language_discriminator']
        ...
        # AMP optimization
            ...
            if self.n_iter % params.accumulate_gradients == 0:
                ...
                if params.clip_grad_norm > 0:
                    ...
                    clip_grad_norm_(apex.amp.master_params(optimizer), params.clip_grad_norm)
    ...
    def mask_out(self, x, lengths):
        ...
        return x, _x_real, pred_mask.bool()
    ...
    def mlm_step(self, lang1, lang2, lambda_coeff):
        ...
        # optimize
        self.optimize(loss, model_name=name)
...
class EncDecTrainer(Trainer):
    ...
    def __init__(self, encoder, decoder, data, params, language_discriminator=None):


        if params.use_language_discriminator:
            self.MODEL_NAMES = ['encoder', 'decoder', 'language_discriminator']
        else:
    ...
    def mt_step(self, lang1, lang2, lambda_coeff):
        ...
        if params.use_language_discriminator:
            self.language_discriminator.train()
        ...
        # size of dec2: [slength, batch_size, dim]
        if params.use_language_discriminator:
            first_time_step_outputs = dec2[0].detach().clone() # size: [batch_size, dim]

            assert lang2_id in [0, 1] # make sure the language id is either 0 or 1
            ground_truth_labels_dae = to_cuda(torch.tensor([lang2_id for i in range(first_time_step_outputs.size(0))], dtype=torch.long, requires_grad=False))[0]
            predictions = self.language_discriminator(first_time_step_outputs)
            
            # computing the loss for the language discriminator and update the loss for the language_discriminator only
            language_discriminator_loss_dae = F.cross_entropy(predictions, ground_truth_labels_dae, reduction='mean')
            # print('language_discriminator_loss_dae: ', language_discriminator_loss_dae)
            self.optimize(language_discriminator_loss_dae, model_name='language_discriminator')
        ...
        # optimize
        self.optimize(loss, model_name='model')
    ...
    def bt_step(self, lang1, lang2, lang3, lambda_coeff):
        ...
        if self.params.use_language_discriminator:
            ...
            enc1 = enc1.transpose(0, 1)
            x2, len2, dec = _decoder.generate(enc1, len1, lang2_id, max_len=int(1.3 * len1.max().item() + 5), return_outputs=True)
            langs2 = x2.clone().fill_(lang2_id)

            # language discriminator for first-time-step outputs 
            first_time_step_outputs = dec[0] # size: [batch_size, dim]
            assert lang2_id in [0, 1] # make sure the intermidiate language id is either 0 or 1
            ground_truth_labels_bt = to_cuda(torch.tensor([lang2_id for i in range(first_time_step_outputs.size(0))], dtype=torch.long, requires_grad=False))[0]
            
            # freeze the language_discriminator
            temp_language_discriminator = copy.deepcopy(self.language_discriminator.module if params.multi_gpu else self.language_discriminator)
            for p in temp_language_discriminator.parameters():
                p.requires_grad = False
            temp_language_discriminator.eval()

            predictions = temp_language_discriminator(first_time_step_outputs)
            # computing the loss and update the main model only
            language_discriminator_loss_bt = F.cross_entropy(predictions, ground_truth_labels_bt, reduction='mean')
            language_discriminator_loss_bt = self.params.language_discriminator_loss_weight * language_discriminator_loss_bt
            # print('language_discriminator_loss_bt: ', language_discriminator_loss_bt)
        ...
        # optimize
        if params.use_language_discriminator:
            final_loss = loss + language_discriminator_loss_bt
            self.optimize(final_loss, model_name='model')
        else:
            self.optimize(loss, model_name='model')
